...
def preemphasis(signals, coeff=0.95):
    ...
    """
    return np.append(signals[0],signals[1:] - coeff * signals[:-1])
...
def add_windows(frames):
    ...
    ###
    win = np.hamming(frame_len)  # Hamming window
    # win = 0.54 - 0.46*np.cos(2*np.pi*np.arange(0,frame_len)/(frame_len-1 )) ###score++
    new_win = np.tile(win,(n_frame,1))
    frames = frames*new_win
    ###
    return frames, win
...
def get_power_spectrum(frames):
    ...
    ###
    mag_spectrum = np.abs(np.fft.rfft(frames, n=frame_len))
...
def get_fbank(power_spectrum, sr, n_filter):
    ...
    ###
    for j in range(1, n_filter + 1):
        f_left = int(bin[j - 1])
        f_center = int(bin[j])
        f_right = int(bin[j + 1])
        for k in range(f_left, f_center):
            fbanks[j - 1, k] = (k - bin[j - 1]) / (bin[j] - bin[j - 1])
        for k in range(f_center, f_right):
            fbanks[j - 1, k] = (bin[j + 1] - k) / (bin[j + 1] - bin[j])
    ###
...
def get_mfcc(fbank,n_mfcc):
    ...
    ## reference1 score++
    dct_matrix = np.zeros((n_filter, n_filter))
    for i in range(n_filter):
        if i == 0:
            a = np.sqrt(1 / (4 * n_filter))
        else:
            a = np.sqrt(1 / (2 * n_filter))
        dct_matrix[i] = 2 * a * np.cos(np.pi * i * (np.arange(n_filter) + 0.5) / n_filter)
    feats = np.dot(fbank, dct_matrix.T)

    ## reference2 score++
    # feats = np.zeros((n_frame, n_filter))
    # for i in range(n_frame):
    ...
    #         else:
    #             a = np.sqrt(1/(2*n_filter))
    #         feats[i][j] = 2*a*np.sum(fbank[i]*np.cos(np.pi*j*(np.arange(n_filter)+0.5)/n_filter))

    ## reference3
    # feats = dct(fbank, type=2,  norm='ortho')
    ###
