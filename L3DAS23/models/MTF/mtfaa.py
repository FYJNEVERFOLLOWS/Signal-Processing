...
import sys
sys.path.append('/Work21/2021/wanghonglong/codeWork/L3DAS23/models/MTF')
...
class MTFAANet(nn.Module):
    ...
    def __init__(self,
        ...
        self.PE = PhaseEncoder(cout=PEc*2*2, n_sig=1, cin=PEc*2)  # PhaseEncoder(PEc, n_sig)
        # 32ms @ 48kHz  
        ...
        C_en = [PEc*2*n_sig] + parse_1dstr(Co) # [PEc//2*n_sig] + parse_1dstr(Co)
        C_de = [4] + parse_1dstr(Co)
        O = parse_1dstr(O)
        # print("C_en:{}".format(C_en))
        # print("C_de:{}".format(C_de))
    ...
    def forward(self, sigs):
        ...
        MC = False
        if sigs.ndim == 3:
            bc, mics, saps = sigs.size()
            sigs = sigs.view(bc*mics, saps).contiguous()
            MC = True
        cspecs = self.stft.transform(sigs)
        # print("cspecs.shape:{}".format(cspecs.shape))
        if MC:
            bm, _, freq_len, seq_len = cspecs.size()
            cspecs = cspecs.view(bc, mics, _, freq_len, seq_len).contiguous()
            cspecs = cspecs.view(bc, mics*_, freq_len, seq_len).contiguous()
        # print("After view cspecs.shape:{}".format(cspecs.shape))     
        # D / E ?
        # print("cspecs.shape:{}".format(cspecs.shape))
        D_cspec = cspecs[:, :2, ...]
        ...
        # print("ERB out.shape:{}".format(out.shape))
        encoder_out = []
        ...
        return th.stack([real, imag], dim=1), self.stft.inverse(real, imag) # mag


def test_nnet():
    # noise supression (microphone, )
    nnet = MTFAANet(n_sig=1, PEc=4)
    inp = th.randn(16, 4, 80000)
    # mag, cspec, wav = nnet([inp])
    cspec, wav = nnet(inp)
    print(cspec.shape, wav.shape)
    # echo cancellation (microphone, error, reference,)
    # AEC用了三个并行的Complex Conv2d,扩展成多通道只是把一个Complex Conv2d通道数增加
...
def test_mac():
    ...
    import torch as th
    nnet = MTFAANet(M = 4, n_sig=3) #n_sig用来控制ComplexConv2d数量和PhaseEncoder输入通道数
    # 增强是单通道的，最后估计mask也只是估计了一个通道的mask
