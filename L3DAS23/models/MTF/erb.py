...
class Banks(nn.Module):
    def __init__(self, nfilters, nfft, fs, low_freq=None, high_freq=None, learnable=False):
        ...
        filter, _ = linear_fbanks.linear_filter_banks(
            ...
            nfft=self.nfft,
            # low_freq=low_freq, #不能直接传None，还是默认吧
            # high_freq=high_freq,
            fs=self.fs,
        )
...
def test_bank():
    ...
    stft = STFT(32*16, 8*16, 32*16, "hann")
    net = Banks(256, 32*16, 16000)
    sig_raw, sr = sf.read("/CDShare3/L3DAS23_Task1/L3DAS23_Task1_dev/L3DAS23_Task1_dev/data/84-121123-0000_A.wav")
