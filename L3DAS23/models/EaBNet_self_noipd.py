...
sys.path.insert(0,"/Work21/2021/wanghonglong/codeWork/L3DAS23/models")
from utils.DFRNN import DPRNNBlock
    
def mulpointwise(in_channels, out_channels):
...
class MicAttention(nn.Module):
    def __init__(self, channels = 32, sel_len = 501, mics = 4):
        super(MicAttention, self).__init__()
        self.mics = mics
        self.key = pointwise(sel_len, channels)
        self.query= pointwise(sel_len, channels)
        self.values= mulpointwise(mics, mics)

    def forward(self, x):
        x_clone = x
        bn, channels, seqlen, freq_len = x.size()
        x_v = x.view(bn // self.mics, self.mics, channels, seqlen, freq_len).permute(0,1,3,2,4).contiguous()
        x_v = x_v.view(bn // self.mics, self.mics,  seqlen,-1).contiguous()
        x_seq = x.transpose(1,2).contiguous()
        k_mat = self.key(x_seq).view(bn//self.mics, self.mics, -1, channels, freq_len).permute(0,3,4,1,2)
        q_mat = self.query(x_seq).view(bn//self.mics, self.mics, -1, channels, freq_len).permute(0,3,4,2,1)
        v_mat = self.values(x_v).view(bn//self.mics, self.mics, -1, channels, freq_len).permute(0,3,4,1,2)
        #print("k_mat.shape:{}, q_mat.shape:{}".format(k_mat.shape, q_mat.shape))
        #print("v_mat.shape:{}".format(v_mat.shape))
        score = torch.matmul(k_mat, q_mat)
        #print("score.shape:{}".format(score.shape))
        probs = nn.Softmax(dim = -1)(score)
        #print(probs[0,0])
        x_res = torch.matmul(probs, v_mat)
        #print("x_res.shape:{}".format(x_res.shape))
        x_res = x_res.permute(0,3,1,4,2).contiguous()
        #print("x_res.shape:{}".format(x_res.shape))
        x_res = x_res.view(bn, channels, seqlen, freq_len).contiguous()
        #print("x_res.shape:{}".format(x_res.shape))
        return x_res


def get_ipd(inpt_stft, ref_mic=0, cosine=True):
    real = inpt_stft[..., 0]
    imag = inpt_stft[..., -1]
    phase = torch.atan2(imag, real)  # batch_size, seq_len, freq_num, mics
    diffs = []
    # print("phase.shape", phase.shape)
    for mic_idx in range(phase.shape[3]):
        if cosine:
            diff = torch.cos((phase[:, :, :, mic_idx] - phase[:, :, :, ref_mic])).unsqueeze(-1)
        else:
            diff = torch.sin((phase[:, :, :, mic_idx] - phase[:, :, :, ref_mic])).unsqueeze(-1)
        diffs.append(diff)

    result = torch.cat(diffs, dim=-1)
    return result

class EaBNet(BaseModel):
    def __init__(self,
                 sample_rate=16000,
                 k1: tuple = (2, 3),
                 k2: tuple = (1, 3),
        ...
        """
        super(EaBNet, self).__init__(sample_rate=sample_rate, in_channels=None)
        self.k1 = k1
        self.k2 = k2
        self.c = c
        self.M = M
        ...
        if is_u2:
            self.en = U2Net_Encoder(2, k1, k2, c, intra_connect, norm_type)  # M*2
            self.de = U2Net_Decoder(embed_dim, c, k1, k2, intra_connect, norm_type)
        else:
            self.en = UNet_Encoder(4, k1, c, norm_type)  # M*2
            self.de = UNet_Decoder(embed_dim, k1, c, norm_type)

        if topo_type == "mimo":
            if bf_type == "lstm":
                self.bf_map = LSTM_BF(embed_dim, M)  # M
            elif bf_type == "cnn":
                self.bf_map = nn.Conv2d(embed_dim, M * 2, (1, 1), (1, 1))  # pointwise
        elif topo_type == "miso":
            self.bf_map = nn.Conv2d(embed_dim, 2, (1, 1), (1, 1))  # pointwise

        self.key = pointwise(501, 32)
        self.query= pointwise(501, 32)
        self.values= mulpointwise(6,6)

    def forward(self, inpt: Tensor, valid_channels=None) -> Tensor:
        """
        :param inpt: (B, T, F, M, 2) -> (batchsize, seqlen, freqsize, mics, 2)
        :return: beamformed estimation: (B, 2, T, F)
        """
        batch_size, mics, wav_len = inpt.size()
        # input = torch.stft(inpt, 320, 0.01, 0.02, torch.hann_window(0.02)
        fft_num = int(320)
        win_shift = int(0.01 * 16000)
        win_size = int(0.02 * 16000)
        inpt = inpt.view(batch_size * mics, wav_len)
        # print("inpt.shape:{}".format(inpt.shape))
        inpt_stft = torch.stft(inpt, fft_num, win_shift, win_size, torch.hann_window(win_size).cuda())
        # print("inpt_stft.shape:{}".format(inpt_stft.shape))
        _, freq_num, seq_len, _ = inpt_stft.shape
        inpt_stft = inpt_stft.view(batch_size, mics, freq_num, seq_len, -1).permute(0, 3, 2, 1, 4)

        # # 从这里计算那mic维度就是3
        # cos_ipd = get_ipd(inpt_stft, 0)
        # # normal_cos_ipd = self.ipd_norm1(cos_ipd.permute(0,3,1,2)).permute(0,2,3,1)
        # sin_ipd = get_ipd(inpt_stft, 0, False)
        # # normal_sin_ipd = self.ipd_norm2(sin_ipd.permute(0,3,1,2)).permute(0,2,3,1)
        # ipds = torch.stack((cos_ipd, sin_ipd), -1)

        # conduct sqrt power-compression
        inpt_mag, inpt_phase = torch.norm(inpt_stft, dim=-1) ** 0.5, torch.atan2(inpt_stft[..., -1], inpt_stft[..., 0])
        # print("inpt_mag.shape:{}".format(inpt_mag.shape))

        inpt_stft = torch.stack((inpt_mag * torch.cos(inpt_phase), inpt_mag * torch.sin(inpt_phase)), dim=-1)

        # print("compressed inpt_stft.shape:{}".format(inpt_stft.shape))

        inpt = inpt_stft
        # inpt = torch.cat((inpt_stft, ipds), -1)
        ...
        x = inpt.permute(0, 3, 1, 2, 4).contiguous()
        x = x.view(b_size * M, seq_len, freq_len, -1).permute(0, 3, 1, 2)  # b_size

        # print("before u encoder x.shape:{}".format(x.shape))
        x, en_list = self.en(x)
        #print("after u encoder x.shape:{}".format(x.shape))
        
        # c = x.shape[1]
        # x = x.transpose(-2, -1).contiguous().view(b_size*M, -1, seq_len)  #b_size
        # x = x.view(b_size, M, -1, seq_len).contiguous()
        # x_seq = x.transpose(1, 3)
        # #print("after view x.shape:{}".format(x.shape))
        # k_mat = self.key(x_seq).permute(0,2,3,1)  #batch,M,256,401->32
        # q_mat = self.query(x_seq).permute(0,2,1,3)    #batch,M,256,401->32
        # v_mat = self.values(x).transpose(1,2)   #batch,M,256,401
        # #print("k_mat.shape:{}, q_mat.shape:{}".format(k_mat.shape, q_mat.shape))
        # #print("v_mat.shape:{}".format(v_mat.shape))
        # score = torch.matmul(k_mat, q_mat)
        # #print("score.shape:{}".format(score.shape))
        # probs = nn.Softmax(dim = -1)(score)
        # #print(probs[0,0])
        # x = torch.matmul(probs, v_mat)
        # #print("after attention x.shape:{}".format(x.shape))
        # x = x.transpose(1,2).contiguous()

        # x = x.view(b_size*M, -1, seq_len).contiguous()
        # x = x.view(b_size*M, c, -1, seq_len).transpose(-2, -1).contiguous()


        # print("before u decoder x.shape:{}".format(x.shape))
        x = self.de(x, en_list)
        # print("after u decoder x.shape:{}".format(x.shape))

        ##mean pooling
        x = x.reshape(b_size, M, self.c * 2, seq_len, freq_len).mean(1).squeeze(1)
        ...
        if self.topo_type == "mimo":
            ...
            esti_x_r, esti_x_i = (bf_w_r*inpt_stft[...,0]-bf_w_i*inpt_stft[...,-1]).sum(dim=-1), \
                                 (bf_w_r*inpt_stft[...,-1]+bf_w_i*inpt_stft[...,0]).sum(dim=-1)
            
            sig_stft = torch.stack((esti_x_r, esti_x_i), dim=1)# [batch, 2, n_frame, fft_size]
            sig_stft = sig_stft.permute(0 ,3, 2, 1).contiguous() # batch, fft_size, n_frame, 2
            # print("sig_stft.shape:{}".format(sig_stft.shape))
            signals = torch.istft(sig_stft, fft_num, win_shift, win_size, torch.hann_window(win_size).cuda())
            signals = signals.reshape(batch_size, wav_len)                     
            return sig_stft.permute(0 ,3, 2, 1).contiguous(), signals
    ...
    def get_model_args(self):
        config = {
            ...
            "intra_connect": self.intra_connect,
...
class U2Net_Encoder(nn.Module):
    def __init__(self,
        ...
        dprnn_net = []
        dprnn_net.append(
            DPRNNBlock(79, 32, 64, "gLN", True, "LSTM", dropout=0.0,
                       )
        )
        dprnn_net.append(
            DPRNNBlock(39,32, 32,"gLN",True,"LSTM",dropout=0.0,
            )
        )
        dprnn_net.append(
            DPRNNBlock(19, 32, 32, "gLN", True, "LSTM", dropout=0.0,
                       )
        )
        self.dprnn_list = nn.ModuleList(dprnn_net)
        ma_list = []
        ma_num = 4
        for i in range(ma_num):
            ma_list.append(MicAttention())
        #ma_list.append(MicAttention(64))
        self.mas = nn.ModuleList(ma_list)

    def forward(self, x: Tensor):
        en_list = []
        for i in range(len(self.meta_unet_list)):
            x = self.meta_unet_list[i](x)
            # print("U encoder each layer:{}".format(x.shape))
            # print("U encoder each layer:{}".format(x.shape))
            ###stream_pooling
            if i <= 2:
                x = self.dprnn_list[i](x)
            # if i == 0:
            #     x = self.dprnn_list[0](x)
            # elif i == 2:
            #     x = self.dprnn_list[1](x)
            #x = stream_pooling(x)
            ###
            x = self.mas[i](x)
            en_list.append(x)
        # print("u encoder before last_conv x.shape:{}".format(x.shape))
        x = self.last_conv(x)
        # print("u encoder after last_conv x.shape:{}".format(x.shape))
        #x = self.mas[i](x)
        en_list.append(x)
        return x, en_list
...
class LSTM_BF(nn.Module):
    ...
    def forward(self, embed_x: Tensor) -> Tensor:
        ...
        bf_w = self.w_dnn(x).view(B, T, F, self.M, 2)  # (B, T, F, self.M, 4)
        return bf_w
...
def main(args, net):
    ...
    target_stft = torch.stft(target_wav, fft_num, win_shift, win_size,
                             torch.hann_window(win_size).to(target_wav.device))
    ...
    target_mag, target_phase = torch.norm(target_stft, dim=1) ** 0.5, torch.atan2(target_stft[:, -1, ...],
                                                                                  target_stft[:, 0, ...])
    noisy_stft = torch.stack((noisy_mag * torch.cos(noisy_phase), noisy_mag * torch.sin(noisy_phase)), dim=-1).cuda()
    target_stft = torch.stack((target_mag * torch.cos(target_phase), target_mag * torch.sin(target_phase)),
                              dim=1).cuda()
...
if __name__ == '__main__':
    ...
    # main(args, net)
    input = torch.randn(2, 4, 80000).cuda()  # (4, 401, 161, 9, 2).cuda()
    cspecs, wav = net(input)
    print(wav.shape)