...
class EaBNet(nn.Module):
    ...
    def forward(self, inpt: Tensor) -> Tensor:
        ...
        x, en_list = self.en(x)
        print("after en : x.shape".format(x.shape))
        c = x.shape[1]
        ...
        if self.topo_type == "mimo":
            ...
            esti_x_r, esti_x_i = (bf_w_r*inpt_stft[...,0,0]-bf_w_i*inpt_stft[...,0,-1]).sum(dim=-1), \
                                 (bf_w_r*inpt_stft[...,0,-1]+bf_w_i*inpt_stft[...,0,0]).sum(dim=-1)
...
class UNet_Encoder(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        for i in range(len(self.unet_list)):
            ...
            print("Each en x.shape:{}".format(x.shape))
            en_list.append(x)
        return x, en_list
...
def com_mag_mse_loss(esti, label, frame_list):
    ...
    if label.ndim == 2:
        ...
        print("label.shape:{}".foramt(label.shape))
    print("esti.shape:{}".foramt(esti.shape))
    mask_for_loss = []
    utt_num = esti.size()[0]
    with torch.no_grad():
...
if __name__ == '__main__':
    ...
    #main(args, net)
    input = torch.randn(4, 4, 80000).cuda()#(4, 401, 161, 9, 2).cuda()
    target = torch.randn(4, 80000).cuda()
    esti_stft, wav = net(input)
    print("esti_stft.shpae:{}".format(esti_stft.shape))
    cmse_loss = LJJComMagMse()
    loss = cmse_loss(esti_stft, target)
    print("Calculated loss value:{}".format(loss.item()))
    print(wav.shape)