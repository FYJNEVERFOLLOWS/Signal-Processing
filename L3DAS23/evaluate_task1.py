...
def enhance_sound(predictors, model, device, length, overlap):
    ...
    for i in range(len(starts)):
        ...
        # print("cut_x.shape:{}".format(cut_x.shape))
        if 'audiovisual' in args.architecture:
            images = images.to(device)
            predicted_x = model(cut_x, images, device)
        else:
            predicted_cspec, predicted_x = model(cut_x)
        predicted_x = predicted_x.cpu().numpy()

        # print("predicted_x.shape:{}".format(predicted_x.shape))
        # Target shape of official code is [1, seqlen]
        if not args.architecture == "mmub":
            predicted_x = np.expand_dims(predicted_x, axis=-2)
...
def main(args):
    ...
    if args.architecture == 'fasnet':
        ...
        model = EaBNet(M=4, is_causal=args.is_causal)
    if args.use_cuda:
            ...
            if metric is not None:
                ...
                if args.save_sounds_freq is not None:
                    ...
                    if count % args.save_sounds_freq == 0:
                        ...
                        sf.write(os.path.join(sounds_dir, str(example_num)+'_clean.wav'), target, 16000, 'PCM_16')
                        sf.write(os.path.join(sounds_dir, str(example_num)+'_noisy.wav'), x[0][0], 16000, 'PCM_16')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--architecture', type=str, default='mmub',
                        help="model name")
    parser.add_argument('--is_causal', type=str, default="False",
                        help="Model is casual or not ")
